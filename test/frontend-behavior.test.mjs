...
test('ngrok previews use the production same-site API host', async () => {
  ...
  const prod = await runConfigForHost('game.renlimeng.qzz.io');
  const localhost = await runConfigForHost('localhost');
  const workersDev = await runConfigForHost('spaceshooter.842695824.workers.dev');

  assert.equal(ngrok.apiBase, 'https://game.renlimeng.qzz.io');
  assert.equal(prod.apiBase, '');
  assert.equal(localhost.apiBase, '');
  assert.equal(workersDev.apiBase, 'https://game.renlimeng.qzz.io');
});
